...
LoginResult LoginUseCase::execute(const std::string& accountID, const std::string& pinCode) const {
    AccountRepository repo;
    Account* acc = repo.searchAccount(accountID);

    if (acc == nullptr)
        return LoginResult::Failure(LoginResultCode::ACCOUNT_NOT_FOUND);

    if (!acc->checkPinCode(pinCode))
