...
namespace array {
    ...
    void ReadArray (int array[], int array_length) {
        // int array_length = validation::ReadPositiveNumber("Enter array array_length (max " + std::to_string(array_length) + "): ");
    ...
    void FillPositiveArrayByUser (int array[], int& array_length) 
    {
        ...
        do
        {
            number = validation::ReadPositiveNumber("\nPlease enter a number:\n");
            AddArrayElement(array, array_length, number);

            addMore = validation::ReadNumberInRange("\nDo you want to add more numbers? [0]: No, [1]: Yes?\n", 0, 1);
            std::cin.get();
