...
namespace libcore::mystring {
    ...
    {
        std::string::size_type pos = text.find(string_toReplace);
    ...
    { 
        ...
        std::cout <<"\nYour string wrords are: \n\n"; std::string::size_type pos = 0; std::string sWord; // define a string variable  // use find() function to get the position of the delimiters  
        while ((pos = text.find(delim)) != std::string::npos)     
        {         
            sWord =text.substr(0, pos); // store the word   
            if (sWord !="")         
    ...
    void TrimRight(std::string& text) {
        ...
        for (std::string::size_type i = text.length(); i > 0; --i) {
            if (text[i - 1] != ' ') {
